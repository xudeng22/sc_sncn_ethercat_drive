...
static unsigned get_configuration_from_dictionary(
        ...
        ConfigParameter_t* Config)
{
    // this is a list of parameter objects which don't have additional subindexes, so we read only the subindex 0
    const uint16_t index_without_subindex[] = {0x2001, 0x6075, 0x6076, 0x6085, 0x6080, 0x607E, 0x607C, 0x2A00, 0x2002, 0x6072, 0x6073, 0x6086};
    const unsigned int num_index_without_subindex = 12;
    // this is a list of parameter objects which have multiple subindexes
    // so we read the number of subindexes at the subindex 0 and then read them all
    const uint16_t index_with_subindex[] = {0x607D, 0x607B, 0x2003, 0x2004, 0x2100, 0x2005, 0x2006, 0x2007, 0x2010, 0x2011, 0x2012, 0x2201, 0x2202, 0x2203, 0x2204, 0x2205, 0x2206, 0x2207, 0x2208};
    const unsigned int num_index_with_subindex = 19;

    int count = 0;
    uint32_t value = 0;
    for (int i=0; i<num_index_without_subindex; i++) {
        value = i_canopen.get_object_value(index_without_subindex[i], 0);
        Config->parameter[count][0].index    = index_without_subindex[i];
        Config->parameter[count][0].subindex = 0;
        Config->parameter[count][0].value    = value;
        count++;
    }
    for (int i=0; i<num_index_with_subindex; i++) {
        unsigned int subindex = i_canopen.get_object_value(index_with_subindex[i], 0);
        for (int j=1; j<=subindex; j++) {
            value = i_canopen.get_object_value(index_with_subindex[i], j);
            Config->parameter[count][0].index    = index_with_subindex[i];
            Config->parameter[count][0].subindex = j;
            Config->parameter[count][0].value    = value;
            count++;
        }
    }
        ...
        client interface i_coe_communication i_canopen)
{
    ...
    result = write_config(CONFIG_FILE_NAME, &Config, i_spiffs);
...
//        debug_print_state(state);
        ...
        if (opmode == OPMODE_NONE) {
            ...
            tuning_handler_ethercat(tuning_command,
                    ...
                    sensor_commutation, sensor_motion_control,
                    send_to_master,
                    i_motion_control, i_position_feedback_1, i_position_feedback_2, i_coe, i_spiffs);
