...
bool ctrl_shutdown(int control_word) {
    return !read_controlword_switch_on(control_word) && read_controlword_enable_voltage(control_word) && read_controlword_quick_stop(control_word);
}
...
bool ctrl_quick_stop(int control_word) {
    return read_controlword_enable_voltage(control_word) && !read_controlword_quick_stop(control_word);
}

bool ctrl_disable_op(int control_word) {
    return read_controlword_switch_on(control_word) && read_controlword_enable_voltage(control_word) && read_controlword_quick_stop(control_word) && !read_controlword_enable_voltage(control_word);
}
...
int get_next_state(int in_state, check_list &checklist, int controlword) {
    ...
    switch(state)
    {
        ...
        case S_READY_TO_SWITCH_ON:
            ...
            if (checklist.fault)
                out_state = S_FAULT;
            else if (checklist.switch_on && ctrl_input && checklist.ready)
                out_state = S_SWITCH_ON
            else if (!checklist.switch_on || !ctrl_input)
                out_state = S_READY_TO_SWITCH_ON;
            else if (!checklist.ready)
                out_state = S_SWITCH_ON_DISABLED
            break;
        ...
        case S_SWITCH_ON:
            ...
            if (checklist.fault)
                ...
                out_state = S_SWITCH_ON;
            else if (checklist.ready && !checklist.switch_on)
                out_state = S_READY_TO_SWITCH_ON
            else if (!checklist.ready)
                out_state = S_SWITCH_ON_DISABLED
            break;
        ...
        case S_OPERATION_ENABLE:
            ...
            if (checklist.fault)
                out_state = S_FAULT;
            else if (!ctrl_input && checklist.switch_on && )
